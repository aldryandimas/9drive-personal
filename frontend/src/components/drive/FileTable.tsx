...
export function FileTable({ files, mode = 'default', selectedFileIds = new Set<string>(), allSelected = false, onFileContextMenu, onToggleFile, onOpenFile, onToggleAll }: { files: FileItem[]; mode?: 'default' | 'shared' | 'recent' | 'starred' | 'archived'; selectedFileIds?: Set<string>; allSelected?: boolean; onFileContextMenu?: (event: MouseEvent<HTMLElement>, file: FileItem) => void; onToggleFile?: (file: FileItem) => void; onOpenFile?: (file: FileItem) => void; onToggleAll?: () => void }) {
  return (
    <div className="mt-5">
      <div className="grid gap-3 sm:hidden">
        ...
        {files.map((file) => {
          ...
          return (
            <article key={file.id ?? file.name} onClick={() => onOpenFile?.(file)} onContextMenu={(event) => onFileContextMenu?.(event, file)} className={selected ? 'overflow-hidden rounded-2xl border border-blue-200 bg-blue-50 p-4 shadow-sm' : 'overflow-hidden rounded-2xl border border-slate-200 bg-white p-4 shadow-sm'}>
      ...
      <table className="w-full min-w-[760px] border-collapse text-left text-sm">
        ...
        <tbody>
          {files.map((file) => (
            <tr key={file.id ?? file.name} onContextMenu={(event) => onFileContextMenu?.(event, file)} onClick={() => onOpenFile?.(file)} className={selectedFileIds.has(file.id ?? '') ? 'border-b border-blue-100 bg-blue-50 transition hover:bg-blue-50' : 'border-b border-slate-200 transition hover:bg-slate-50'}>
